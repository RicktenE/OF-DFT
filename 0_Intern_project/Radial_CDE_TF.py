...
#x_end_array = [1, 3, 7, 100]
for i in range(4):
    x_end = x_end_array[i]

    #rs = np.arange(0.1, 25.0, 0.01)
    rs = np.arange(1e-2, x_end, 1e-2)
    radius = rs[-1]
    r_inner = 0.0
    ...
    #    plotting_sqrt(u_nk, 'Density TFDW PRE solver')
       ...
       #bc_R_du = DirichletBC(W.sub(1), (0), boundary_R)
        bc_R_du = DirichletBC(W.sub(0), (0), boundary_R)
        bcs_du = [bc_R_du]
...
#        print('Check end of loop, iteration: ', iters)
    ...
    psi = u_n**(2/3)*r*(3*math.pi**2/(2*np.sqrt(2)))**(2/3)
    gr = project(psi.dx(0),V)    
