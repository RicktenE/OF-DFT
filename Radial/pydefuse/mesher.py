...
def tetrahedralization_tetgen(points, facets = None, holes = None, extra_surf_points=False):
    ...
    if facets == None and holes == None:
        node_f = tempfile.NamedTemporaryFile(suffix=".node", delete=False, encoding='utf-8', mode='w+');
        style='node'
    else:
        if len(facets) == 1:
            node_f = tempfile.NamedTemporaryFile(suffix=".smesh", delete=False, encoding='utf-8', mode='w+');
            style='smesh'
        else:
            node_f = tempfile.NamedTemporaryFile(suffix=".poly", delete=False, encoding='utf-8', mode='w+');
            style='poly'
